#!/usr/bin/env python3
"""
Extract citation contexts from IEP articles for a specific paper.

Given a paper (author, year) and an IEP entry, finds all citations to that paper
and extracts the surrounding context and attributed claims.

Usage:
    python get_iep_context.py freewill --author "Frankfurt" --year 1971
    python get_iep_context.py compatibilism --author "Fischer" --year 1998 --coauthor "Ravizza"
...
def log_progress(message: str) -> None:
    ...
    print(f"[get_iep_context.py] {message}", file=sys.stderr, flush=True)
...
def build_citation_patterns(author: str, year: str, coauthor: Optional[str] = None) -> list[re.Pattern]:
    """Build regex patterns to match citations.

    IEP typically uses similar citation formats to SEP:
    - Frankfurt (1971)
    - Frankfurt 1971
    - (Frankfurt 1971)
    """
...
def extract_claims(contexts: list[dict]) -> list[dict]:
    """Attempt to extract philosophical claims from each context."""
    claim_patterns = [
